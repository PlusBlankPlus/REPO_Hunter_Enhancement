...
namespace HunterMod
{
    ...
    public class Plugin : BaseUnityPlugin
    {
        ...
        public static ConfigEntry<string> WanderSoundBorrowTarget { get; private set; }
        ...
        private void Awake()
        {
            ...
            // *** Updated Description for Volume ***
            ...
            WanderSoundBorrowTarget = Config.Bind("G. Wander Sound", "WanderSoundBorrowTarget", "soundFootstepShort", "The name of a Sound field on the EnemyHunterAnim component to borrow for playing the wander sound (e.g., soundFootstepShort, soundMoveShort). The 3D properties (falloff, occlusion) of this borrowed sound type will be used.");
            ...
            if (EnableWanderSound.Value && !string.IsNullOrWhiteSpace(WanderSoundFileName.Value)) { StartCoroutine(LoadCustomAudioClip(WanderSoundFileName.Value)); }
            else if (EnableWanderSound.Value) { LogWarningF("Wander sound enabled but WanderSoundFileName is empty. Wander sound disabled."); }
        ...
        public static void CallShootRPC(EnemyHunter instance) { if (!PhotonNetwork.IsMasterClient) return; PhotonView pv = GetPhotonView(instance); if (pv == null) { LogErrorF($"CallShootRPC: PhotonView is null for {instance?.gameObject?.name}! Cannot send RPC."); return; } if (instance == null || instance.gunAimTransform == null) { LogErrorF($"CallShootRPC: instance or gunAimTransform is null!"); return; } Vector3 intendedTargetPosition = instance.gunAimTransform.position + instance.gunAimTransform.forward * 50f; try { var investigatePointField = AccessTools.Field(typeof(EnemyHunter), "investigatePoint"); if (investigatePointField != null) { Vector3 currentInvestigatePoint = (Vector3)investigatePointField.GetValue(instance); float radius = Vector3.Distance(instance.transform.position, currentInvestigatePoint) > 10f ? 0.5f : 1f; LayerMask hitMask = LayerMask.GetMask("Player", "Default", "PhysGrabObject", "Enemy"); LayerMask visionObstructMask = LayerMask.GetMask("Default"); RaycastHit hitInfo; if (Physics.SphereCast(instance.gunAimTransform.position, radius, instance.gunAimTransform.forward, out hitInfo, 50f, hitMask)) { if (!Physics.Linecast(instance.gunAimTransform.position, hitInfo.point, visionObstructMask)) { intendedTargetPosition = hitInfo.point; } else if (Physics.Raycast(instance.gunAimTransform.position, instance.gunAimTransform.forward, out hitInfo, 50f, hitMask)) { intendedTargetPosition = hitInfo.point; } } else if (Physics.Raycast(instance.gunAimTransform.position, instance.gunAimTransform.forward, out hitInfo, 50f, hitMask)) { intendedTargetPosition = hitInfo.point; } } else { LogWarningF($"CallShootRPC: Could not access investigatePoint field for {instance.gameObject.name}!"); } } catch (System.Exception ex) { LogErrorF($"CallShootRPC: Error calculating target position: {ex.Message}"); } Vector3 finalTargetPosition = intendedTargetPosition; if (EnableRecoil.Value && UnityEngine.Random.value < RecoilChance.Value) { Vector3 randomOffset = UnityEngine.Random.insideUnitSphere * UnityEngine.Random.Range(0f, MaxRecoilOffset.Value); finalTargetPosition = intendedTargetPosition + randomOffset; LogDebugF($"CallShootRPC (Master): Recoil applied! Original: {intendedTargetPosition}, Final: {finalTargetPosition}"); } try { LogDebugF($"CallShootRPC (Master): Sending ShootRPC for {instance.gameObject.name} targeting {finalTargetPosition}"); pv.RPC("ShootRPC", RpcTarget.All, finalTargetPosition); } catch (System.Exception ex) { LogErrorF($"CallShootRPC (Master): Failed to send ShootRPC for {instance.gameObject.name}: {ex}"); } }
